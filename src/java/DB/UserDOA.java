...
public class UserDOA {
    ...
    String search_per = "SELECT  * FROM userpermetion WHERE uname = ? , perm_id = ? and states =1 ";
    ...
    public boolean searchPerm(String uname , String perm) {
       
        PreparedStatement preparedStmt;
        try {
            preparedStmt = con.prepareStatement(search_per);
    
            preparedStmt.setString(1, uname);
            preparedStmt.setString(2, perm);
            ResultSet rs = preparedStmt.executeQuery();
            if (rs.next()) {
                return true ;
            }
